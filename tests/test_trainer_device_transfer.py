...
class FakeAccelerator:
    ...
    def clip_grad_norm_(self, parameters: Any, max_norm: float, norm_type: float = 2.0):
        del parameters, max_norm, norm_type
...
def install_fake_accelerate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_build_fsdp_plugin(fsdp):
        if isinstance(fsdp, int):
            if fsdp == 1:
                return FakeFSDPPlugin(
        ...
        if isinstance(fsdp, dict):
            return FakeFSDPPlugin(**fsdp)
        raise TypeError(f"unexpected fsdp config: {fsdp!r}")

    monkeypatch.setattr(trainer_module, "build_fsdp_plugin", fake_build_fsdp_plugin)
