...
void init_cublas_handle() {
  if (g_handle == nullptr) {
    ...
    if (status != CUBLAS_STATUS_SUCCESS) {
      fprintf(stderr, "Failed to set cuBLAS math mode\n");
      exit(EXIT_FAILURE);
    }
  }
}
...
void destroy_cublas_handle() {
  if (g_handle != nullptr) {
    ...
    if (status != CUBLAS_STATUS_SUCCESS) {
      fprintf(stderr, "Failed to destroy cuBLAS handle\n");
    }
    g_handle = nullptr;
  }
}

void cublas_entsor_op_nn(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
