...
__global__ void __launch_bounds__(256)
    hgemm_wmma_m16n16k16_mma4x2_warp4x4_stages_dsmem_kernel(
        half *A, half *B, half *C,
        int M, int N, int K) {
  const int bx = blockIdx.x + blockIdx.z * gridDim.x * ((int)BLOCK_SWIZZLE);
  const int by = blockIdx.y;
  const int NUM_K_TILES = div_ceil(K, WMMA_K * WARP_TILE_K);
  const int BM = WMMA_M * WARP_TILE_M * WMMA_TILE_M;
  const int BN = WMMA_N * WARP_TILE_N * WMMA_TILE_N;
  const int BK = WMMA_K * WARP_TILE_K;
  extern __shared__ half smem[];
  half *s_a = smem;
  half *s_b = smem + K_STAGE * BM * (BK + A_PAD);
  constexpr int s_a_stage_offset = BM * (BK + A_PAD);
  ...
  const int warp_m = warp_id / WMMA_TILE_N;
  const int warp_n = warp_id % WMMA_TILE_N;
  // BM*BK = 16*4*4*16 = 4096, 4096/256 = 16
  int load_smem_a_m = tid / 1; // 一行BK 16个,每个线程加载16个元素 ,一行一个线程
  int load_smem_a_k = 0;
  int load_smem_b_k = tid / 16; // 一行元素BN是128个,每个线程加载8个,一行16个线程
  int load_smem_b_n = (tid & 15) << 3;
...
#pragma unroll
  for (int k = (K_STAGE - 1); k < NUM_K_TILES; ++k) {
    ...
    uint32_t load_smem_a_ptr =
        smem_a_base_ptr +
        (smem_sel * s_a_stage_offset + load_smem_a_m * (BK + A_PAD) + load_smem_a_k) * sizeof(half);
    uint32_t load_smem_b_ptr =
        smem_b_base_ptr +
        (smem_sel * s_b_stage_offset + load_smem_b_k * (BN + B_PAD) + load_smem_b_n) * sizeof(half);
    CP_ASYNC_CG(load_smem_a_ptr, &A[load_gmem_a_addr], 16);
    CP_ASYNC_CG(load_smem_a_ptr + 16, &B[load_gmem_a_addr + 8], 16);
    CP_ASYNC_CG(load_smem_b_ptr, &B[load_gmem_b_addr], 16);
    CP_ASYNC_COMMIT_GROUP();
#pragma unroll
    for (int warp_k = 0; warp_k < WARP_TILE_K; ++warp_k) {
      wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half,
                     wmma::row_major>
          a_frag[WARP_TILE_M];
      wmma::fragment<wmma::matrix_b, WMMA_K, WMMA_N, WMMA_K, half,
                     wmma::row_major>
          b_frag[WARP_TILE_N];
      const int warp_smem_k = warp_k * WMMA_K;
#pragma unroll
      for (int i = 0; i < WARP_TILE_M; ++i) {
        const int warp_smem_a_m = warp_m * (WMMA_M * WARP_TILE_M) + i * WMMA_M;
        half *load_smem_a_frag_ptr =
            s_a + smem_sel * s_a_stage_offset + warp_smem_a_m * (BK + A_PAD) + warp_smem_k;
        wmma::load_matrix_sync(a_frag[i], load_smem_a_frag_ptr, BK + A_PAD);
      }
#pragma unroll
      for (int j = 0; j < WARP_TILE_N; ++j) {
        ...
        half *load_smem_b_frag_ptr =
            s_b + smem_sel * s_b_stage_offset + warp_smem_k * (BN + B_PAD) + warp_smem_b_n;
...
#pragma unroll
        for (int i = 0; i < WARP_TILE_M; ++i) {
          const int warp_smem_a_m = warp_m * (WMMA_M * WARP_TILE_M) + i * WMMA_M;
          half *load_smem_a_frag_ptr =
              s_a + stage_sel * s_a_stage_offset + warp_smem_a_m * (BK + A_PAD) + warp_smem_k;
          wmma::load_matrix_sync(a_frag[i], load_smem_a_frag_ptr, BK + A_PAD);
        }
#pragma unroll
        for (int j = 0; j < WARP_TILE_N; ++j) {
          const int warp_smem_b_n = warp_n * (WMMA_N * WARP_TILE_N) + j * WMMA_N;
          half *load_smem_b_frag_ptr =
              s_b + stage_sel * s_b_stage_offset + warp_smem_k * (BN + B_PAD) + warp_smem_b_n;
          wmma::load_matrix_sync(b_frag[j], load_smem_b_frag_ptr, BN + B_PAD);
        }
#pragma unroll
        for (int i = 0; i < WARP_TILE_M; ++i) {
...
#pragma unroll
      for (int j = 0; j < WARP_TILE_N; ++j) {
        const int gmem_c_m = by * BM + warp_m * (WMMA_M * WARP_TILE_M) + i * WMMA_M;
        const int gmem_c_n = bx * BN + warp_n * (WMMA_N * WARP_TILE_N) + j * WMMA_N;
        wmma::store_matrix_sync(C + gmem_c_m * N + gmem_c_n, acc[i][j], N, wmma::mem_row_major);
      }
    }
  }
}
