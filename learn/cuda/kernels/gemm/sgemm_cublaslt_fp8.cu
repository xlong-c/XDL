...
int run_fp8_demo(const int m, const int n, const int k) {
  ...
  const std::vector<__nv_fp8_e4m3> h_a = quantize_vector<__nv_fp8_e4m3>(h_a_src);
  const std::vector<__nv_fp8_e4m3> h_b = quantize_vector<__nv_fp8_e4m3>(h_b_src);
  ...
  __nv_fp8_e4m3 *d_a = malloc_and_copy_to_device(h_a);
  __nv_fp8_e4m3 *d_b = malloc_and_copy_to_device(h_b);
  ...
  CHECK_CUBLAS(cublasLtMatrixLayoutCreate(&a_desc, CUDA_R_8F_E4M3, m, k, m));
  ...
  if (returned_results == 0) {
    std::fprintf(stderr, "未找到可用的 fp8 cublasLt heuristic.\n");
    return EXIT_FAILURE;
  }
  ...
  std::printf("cuBLASLt FP8 GEMM benchmark\n");
  std::printf("input_type  = fp8(e4m3), output_type = bf16\n");
