...
//
// 代价是 shared memory 占用随 Br * head_dim 增长，因此当前 V3 只面向
...
__global__ void
  ...
  static_assert(kHeadDim <= 256,
                "V3 caches the full Q tile in shared memory; head_dim > 256 "
  ...
  // V3: Q_tile 不随 tile_K_seqlen 变化，先把完整 Q[Br, d] 读入 shared
...
void launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v3(
    torch::Tensor Q, torch::Tensor K, torch::Tensor V, torch::Tensor O) {
  // V3 专门验证 Ada(sm_89) 上的大 tile 是否导致寄存器 cliff。
  // 因此这里统一使用 64x64 block tile，优先换更低寄存器压力与更高 occupancy。
  ...
  static_assert(smem_max_size <= 98304,
                "V3 full-Q smem layout exceeds the configured dynamic smem "
                "limit.");
...
void flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v3(
  ...
  if (stages > 1) {
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v3<32,
                                                                          2>(
          Q, K, V, O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v3<64,
                                                                          2>(
          Q, K, V, O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v3<96,
                                                                          2>(
          Q, K, V, O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v3<128,
                                                                          2>(
          Q, K, V, O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v3<256,
                                                                          2>(
          Q, K, V, O);
      break;
    default:
      throw std::runtime_error("V3 暂只支持 head_dim <= 256");
      break;
    }
  } else {
    switch (d) {
