...
// 这是一个基于 Tensor Core 与 MMA PTX 的 FlashAttention-2 实验实现 V2。
...
// V2 实验目标：
// 1. 把完整 Q_tile[Br, d] 常驻 shared memory，避免每个 K/V tile 重复从 HBM
//    读取同一个 Q_tile；
// 2. V 单独占用 shared memory，不再复用 Q 缓冲；
// 3. V 阶段每个 16 列 chunk 内的两个 8 列 MMA 都消费完后，再等待下一组预取。
//
// 代价是 shared memory 占用随 Br * head_dim 增长，因此当前 V2 只面向
// head_dim <= 256 的常见训练/推理形状。
...
__global__ void __launch_bounds__(WARP_SIZE *kMmaTileSeqLenQ *kMmaTileSeqLenK)
    flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v2_kernel(
  ...
  static_assert(kHeadDim <= 256,
                "V2 caches the full Q tile in shared memory; head_dim > 256 "
                "would exceed the intended smem budget.");
  ...
  constexpr int Q_tile_size =
      Br * (kHeadDim + kPadQ);                              // 完整 Q tile 的 shared memory 元素数
  ...
  half *K_tile_smem = Q_tile_smem + Q_tile_size;            // K 接在完整 Q tile 后面
  half *V_tile_smem = K_tile_smem + kStage * K_tile_size;   // V 单独占一片 smem，避免覆盖 Q
  ...
  // V2: Q_tile 不随 tile_K_seqlen 变化，先把完整 Q[Br, d] 读入 shared
  // memory。后续扫描所有 K/V tile 时都复用这一份 Q，减少 HBM 读流量。
#pragma unroll
  for (int tile_Q_d = 0; tile_Q_d < (kHeadDim / kMmaAtomK); ++tile_Q_d) {
    int load_gmem_Q_d = tile_Q_d * kMmaAtomK + load_smem_Q_d;
...
#pragma unroll
    for (int i = 0; i < (kMmaAtomK / (kNumThreads / Br)); i += 8) {
      uint32_t load_smem_Q_ptr =
          (smem_Q_base_ptr +
           (load_smem_Q_Br * (kHeadDim + kPadQ) + tile_Q_d * kMmaAtomK +
...
#pragma unroll 1
  for (int tile_K_seqlen = 0; tile_K_seqlen < Tc; ++tile_K_seqlen) {
    ...
    // 预取本轮将要使用的 K 子块：全局内存 -> shared memory。
    // Q 已经在进入 K/V tile 循环前常驻 shared memory。
...
#pragma unroll
    for (int tile_K_d = 0; tile_K_d < (kHeadDim / kMmaAtomK); ++tile_K_d) {
      ...
      // stage>1 时，一边算当前子块，一边预取下一个 K 子块。
      if constexpr (kStage > 1) {
...
#pragma unroll
      ...
      { // kWarpTileSeqLenQ = 1，对应 Q[Br, d] 的一个 warp 子块
        ...
        uint32_t lane_smem_Q_ptr =
            (smem_Q_base_ptr +
             (lane_smem_Q_Br * (kHeadDim + kPadQ) + tile_K_d * kMmaAtomK +
...
#pragma unroll
        ...
        // 以降低 R_D 占用的寄存器数量。
        if constexpr (kStage > 1) {
          // V 每次预取 16 列，j 和 j+1 各消费其中 8 列。
          // 等两个 8 列 MMA 都完成后再等待下一组预取，减少同步并延长 overlap。
          if ((j % 2 == 1) && (((j / 2) + 1) < (kWarpTileHeadDimV / 2))) {
            CP_ASYNC_WAIT_GROUP(kStage - 2);
            __syncthreads();
          }
        }
...
void launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v2(
  ...
  // V2 让完整 Q tile 常驻 shared memory；K/V 仍按 16 列子块做 stage 缓冲。
  constexpr int Q_smem_size = Br * (kHeadDim + kPadQ);
  constexpr int K_smem_size = kStage * (Bc * (kMmaAtomK + kPadK));
  constexpr int V_smem_size = kStage * (Bc * (kMmaAtomN * 2 + kPadV));
  constexpr int smem_max_size =
      (Q_smem_size + K_smem_size + V_smem_size) * sizeof(half);
  static_assert(smem_max_size <= 98304,
                "V2 full-Q smem layout exceeds the configured dynamic smem "
                "limit.");
...
void flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v2(
  ...
  if (stages > 1) {
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v2<32,
                                                                          2>(
          Q, K, V, O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v2<64,
                                                                          2>(
          Q, K, V, O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v2<96,
                                                                          2>(
          Q, K, V, O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v2<128,
                                                                          2>(
          Q, K, V, O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_acc_f32_swizzle_qkv_v2<256,
                                                                          2>(
          Q, K, V, O);
      break;
    default:
      throw std::runtime_error("V2 暂只支持 head_dim <= 256");
      break;
    }
  } else {
    switch (d) {
