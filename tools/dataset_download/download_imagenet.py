...
def download_imagenet_full(save_path, split="validation"):
    ...
    print(f"预计大小: 验证集 ~6.5GB | 训练集 ~140GB")
    print(f"{'='*60}\n")
...
def download_imagenet_wds(save_path):
    ...
    print(f"\n{'='*60}")
    print(f"下载 ImageNet-1K (WebDataset 格式)")
    print(f"目标路径: {save_path}")
    print(f"预计大小: ~150GB")
    print(f"{'='*60}\n")
...
def convert_to_torchvision_format(huggingface_path, output_path):
    ...
    """
    print(f"\n{'='*60}")
    print(f"转换数据集格式为 torchvision 兼容格式")
...
def main():
    ...
    if args.dataset == "full":
        dataset = download_imagenet_full(args.save_path, args.split)
    elif args.dataset == "wds":
        dataset = download_imagenet_wds(args.save_path)
    elif args.dataset == "tiny":
        dataset = download_tiny_imagenet(args.save_path)
    elif args.dataset == "subset":
        dataset = download_imagenet_subset(args.save_path, args.num_samples)
    ...
    if args.convert:
        ...
        print(f"   from torchvision.datasets import ImageNet")
