...
from xdl.callbacks.logging_callback import LoggingCallback, SystemStatsCallback
...
class VAEModel(CoreModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 计算损失
        total_loss, recon_loss, kld_loss = self.loss_function(recon_batch, data, mu, logvar)
...
def visualize_results(model: VAEModel, test_loader: DataLoader, n_samples: int = 8):
    ...
    plt.tight_layout()
    plt.savefig('./others/results/vae_results.png', dpi=150, bbox_inches='tight')
    plt.show()
...
def visualize_latent_space(model: VAEModel, test_loader: DataLoader, n_points: int = 1000):
    ...
    plt.tight_layout()
    plt.savefig('./others/results/vae_latent_space.png', dpi=150, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print(f"训练参数: batch_size={batch_size}, epochs={max_epochs}, lr={learning_rate}")
    
    # 获取数据
    print("加载MNIST数据集...")
    train_loader, val_loader, test_loader = get_mnist_dataloaders(batch_size=batch_size)
    print(f"训练集大小: {len(train_loader)}, 验证集大小: {len(val_loader)}")
    
    # 创建模型
    print("创建VAE模型...")
    model = VAEModel(input_dim=784, hidden_dim=256, latent_dim=2)  # 使用2维潜在空间便于可视化
