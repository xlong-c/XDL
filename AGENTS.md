...
- **`xdl/`** - 核心框架代码
- **`config/`** - YAML配置文件
- **`tools/`** - 数据处理工具集（图像、视频、数据集下载等）
- **`scripts/`** - 安装和环境配置脚本
- **`examples/`** - 示例代码
- **`tests/`** - 单元测试
- **`docs/`** - 详细文档
- **`learn/`** - 学习资源和实验代码
- **`others/`** - 日志、检查点、实验结果等非代码文件

### 实验相关

- **`train_GAN.py`** - GAN训练脚本
- **`train_VAE.py`** - VAE训练脚本
- **`vae.ipynb`** - VAE实验Notebook

## 注意事项

1. **注册系统**: 所有组件必须通过 `@XXX.register_module()` 装饰器注册，否则无法通过配置文件构建

2. **配置验证**: 配置文件会经过严格的模式验证，确保所有必需的参数都存在且类型正确

3. **GPU内存**: 大模型训练时注意使用梯度累积、混合精度训练（AMP）、检查点激活等技术节省显存

4. **回调顺序**: 多个回调按优先级执行，优先级相同时按注册顺序执行

5. **检查点**: 定期保存检查点，包含模型状态、优化器状态、学习率调度器状态和训练进度

6. **分布式训练**: 使用 DeepSpeed 或 Accelerate 时，确保所有进程同步保存检查点

7. **日志记录**: 训练过程中记录关键指标，便于后续分析和调试

8. **代码规范**: 遵循项目代码规范，保持代码整洁和可维护性
