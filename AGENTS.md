...
- **XQT** - 模型压缩与部署实验目录 (图变换, 量化, 剪枝, 导出, benchmark). 部署侧.
...
编写训练入口前, 优先先看 `train_VAE.py`, `train_TwinFlow.py`, `examples/*finetune.py` 和 `xdl/trainer/{trainer.py,coreModel.py}`. 关键事实:
...
当前 v0.x 开发期, 未到 v1.0, 允许破坏性重构, 不需要兼容老接口: 改 API 直接改, 改 recipe schema 直接打破兼容, 删模块/改名/合并/拆分都可以. 取舍顺序: 清晰 > 简洁 > 方便 > 兼容.
