...
- `GradientCheckpointingCallback`: `on_fit_start` 时调用模型的
  `gradient_checkpointing_enable()` / `set_gradient_checkpointing(True)`.
- `ActivationOffloadCallback`: 用 `saved_tensors_hooks` 包住每个训练步,
  把超过 `min_bytes` (默认 32MB) 的 CUDA 激活同步卸载到 pinned CPU;
  训练步异常时通过 `on_exception` 自动清理上下文.

两个回调都是 `xdl.utils.memory` 工具的薄封装; 不想走回调时可以直接
`from xdl.utils import activation_offload_context, enable_gradient_checkpointing, global_grad_norm` 显式调用.
