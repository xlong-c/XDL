...
class CallbackList:
    ...
    def invoke_callbacks(
        ...
        for callback in self.callbacks:
            ...
            try:
                ...
                # 快速失败模式: 全局开关或回调自身声明 fast_fail
                callback_fast_fail = self.fast_fail or bool(
                    getattr(callback, "fast_fail", False)
                )
                if callback_fast_fail:
                    raise TrainingError(
    ...
    def load_checkpoint(
        ...
        for callback in self.callbacks:
            ...
            try:
                ...
                if self.fast_fail or bool(getattr(callback, "fast_fail", False)):
                    raise TrainingError(
                        f"Callback {callback.__class__.__name__}.on_load_checkpoint "
                        f"failed in fast_fail mode: {e}"
                    ) from e

    def _new_errors(self) -> List[Dict[str, Any]]:
        """返回自上次 report/raise 以来新增的错误记录."""
        pending = self._error_count - self._reported_error_count
        if pending <= 0:
            return []
    ...
    def report_epoch_errors(self) -> None:
        """汇总并打印自上次调用以来新增的回调失败."""
        new_errors = self._new_errors()
        if not new_errors:
            return
        self._reported_error_count = self._error_count
        summary = "; ".join(
            f"{error['callback']}.{error['hook']}: {error['error']}"
            for error in new_errors
        )
        self._logger.error("本周期有 %d 个回调失败: %s", len(new_errors), summary)
        print(f"[CallbackList] 本周期有 {len(new_errors)} 个回调失败:")
        for error in new_errors:
            print(f"  - {error['callback']}.{error['hook']}: {error['error']}")

    def raise_errors(self) -> None:
        """把自上次调用以来新增的回调失败以 TrainingError 抛出."""
        new_errors = self._new_errors()
        if not new_errors:
            return
        self._reported_error_count = self._error_count
        ...
        raise TrainingError(f"本周期有 {len(new_errors)} 个回调失败: {summary}")
