...
class RolloutCallback(Callback):
    ...
    * ``core_module.ref_model`` (optional) — reference model for
    ...
    def _compute_ref_logps(
        ...
        if self._rollout_fn is None:
            # If no rollout fn, user must provide ref_logps externally
            return torch.zeros(rollout_latents.shape[0], rollout_latents.shape[1])
        return torch.zeros(rollout_latents.shape[0], self.num_rollouts_per_prompt)
