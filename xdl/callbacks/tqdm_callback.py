...
class TqdmCallback(Callback):
    ...
    def on_train_epoch_start(self, trainer: 'Trainer', core_module: 'CoreModel') -> None:
        ...
        current_epoch = getattr(trainer.state, 'current_epoch', getattr(core_module, 'current_epoch', 1))
    ...
    def on_validation_epoch_start(self, trainer: 'Trainer', core_module: 'CoreModel') -> None:
        ...
        current_epoch = getattr(trainer.state, 'current_epoch', getattr(core_module, 'current_epoch', 0))
    ...
    def _create_progress_bar(
        ...
        if epoch is not None:
            desc = f"Epoch {epoch} - {description}"
        else:
            desc = description
    ...
    def _close_all_progress_bars(self):
        """关闭所有进度条"""
        with self._lock:
            for bar_id, progress_bar in self._progress_bars.items():
                try:
                    progress_bar.close()
                except Exception:
                    pass
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_batch_time': sum(self._batch_times) / len(self._batch_times) if self._batch_times else 0.0,
        }
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        # 重置批次计数
        self._batch_counts = {stage: 0 for stage in ['train', 'val', 'test']}
    ...
    def __repr__(self) -> str:
        """返回TqdmCallback的详细表示"""
        return (f"TqdmCallback(log_frequency={self.log_frequency}, "
                f"show_metrics={self.show_metrics}, "
                f"leave={self.leave}, "
                f"disable={self.disable})")
