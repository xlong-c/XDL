...
class GridImageDataset(Dataset):
    ...
    def __init__(self, csv_file, base_dir="/root/autodl-tmp", grid_rows=None):
        ...
        self.pixel_transform = A.Compose([
            ...
            A.Affine(scale=(0.5, 1), translate_percent={
                     "x": (-0.1, 0.1), "y": (-0.1, 0.1)}, rotate=(-10, 10), p=0.8),
            A.OneOf(
                [
    ...
    def refer_imgaug(self, image):
        ...
        image = self.norm(self.to_tensor(results["image"]/255.))
        return image
    ...
    def imgaug(self, source_image, target_image):
        ...
        source_image = cv2.resize(cv2.cvtColor(
            source_image, cv2.COLOR_BGR2RGB), [512, 512])
        target_image = cv2.resize(cv2.cvtColor(
            target_image, cv2.COLOR_BGR2RGB), [512, 512])
        results = self.pixel_transform(image=source_image, image0=target_image)
        source_image, target_image = self.norm(self.to_tensor(
            results["image"]/255.)), self.norm(self.to_tensor(results["image0"]/255.))
    ...
    def __getitem__(self, idx):
        ...
        if self.grid_rows is None:
            ...
            if self.grid_rows != img_total.shape[0] // self.tile_height:
                print(f"警告: 期望行数 {self.grid_rows} 与实际行数 {img_total.shape[0] // self.tile_height} 不匹配")
        ...
        available_rows = [row for row in range(
            actual_grid_rows) if row != selected_row]
