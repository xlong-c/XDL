...
本文承接 `XDL` 当前仍有效的中短期方向. 它只保留仍然成立的改进项, 不重复现状说明或历史已完成任务.
...
- 在仓库根 `README.md` 或兼容详细页中只保留一段最短 runnable 代码

## P1 - 质量提升

### `xdl/dataset/hair/` 的毛发数据集增强逻辑去重
...
### tests 覆盖继续向 trainer / callbacks / optimizer 扩展
...
### 配置系统继续补强 dataset / collate / task 级抽象
