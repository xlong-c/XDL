...
- [x] 主推黄金路径: `weight_only_tilelang_onnx_tensorrt_golden.yaml` 声明 weight-only low-bit -> TileLang -> benchmark/analyze -> ONNX -> TensorRT deploy. TensorRT target 是 dry-run workflow, 真实 engine / performance 验收仍需目标硬件.
...
- deploy stage 已能创建 ONNX Runtime `InferenceSession` 或 TensorRT runtime session. TensorRT producer 拒绝 dry-run engine, 并记录 engine deserialization / execution-context creation 的验证 metadata; `tests/xqt/test_onnx_optimizer.py` 覆盖两类 producer.
...
- `xqt/recipes/smoke/weight_only_tilelang_onnx_tensorrt_golden.yaml` 声明主推 stage 顺序, `tests/xqt/test_golden_path_recipe.py` 覆盖 typed spec 和 dry-run hardware boundary. TileLang CUDA tests 还会拒绝已知不兼容的 packed-tensor runtime ABI, 相关守卫由 `tests/xqt/test_tilelang_capability_preflight.py` 覆盖.

## 后续验证

1. 在目标 CUDA GPU 的 runtime-compatible TileLang 环境运行 correctness / benchmark tests, 再根据测量结果调整 maturity 或性能宣传. 本机 TileLang `0.1.11` 的 packed-tensor ABI 与当前 XQT kernels 不兼容, 只能验证显式 guard 和 wrapper fallback, 不能作为 kernel correctness 或性能结果.
2. 在目标 TensorRT 版本上执行 golden workflow 的 non-dry-run deploy, 并用 materialized runtime handle 做数值与性能验收.
3. 新增 engine 或 backend 时, 先更新 capability maturity, recipe hardware constraints 和对应的 StageReport assertions.
