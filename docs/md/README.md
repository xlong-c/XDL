...
- 本仓库在 `codebase-memory-mcp` 中的项目名为 `xdl`.
- 使用 `search_graph`, `trace_path`, `get_code_snippet`, `query_graph`, `get_architecture`, `detect_changes`, `index_status` 等 MCP 图谱工具时, `project` 参数统一传 `xdl`.
- 需要重建索引时,对仓库根目录 `/root/workspace/xdl/xdl` 运行 `index_repository`,项目名传 `xdl`.
