# FLUX.2 klein NVFP4 后端推理

本文说明 XQT 对 `black-forest-labs/FLUX.2-klein-4b-nvfp4` 的模型侧接入方式. 该仓库是 FLUX.2 [klein] 4B 的 NVFP4 single-file 权重, XQT 不接管训练, 数据集或任务评测, 只负责把已加载模型中的 packed NVFP4 Linear 模块路由到 operator backend.
...
for backend in ("cutedsl", "cutile", "tilelang"):
    ...
    print(backend, result.target_count)
```
...
- `materialize_flux2_klein_nvfp4_backend()` 默认扫描 `pipeline.transformer`; 如果传入的是 `nn.Module`, target path 相对该模块.
